...
namespace
{
    ...
    std::vector<std::pair<std::string, std::shared_ptr<hub_message_base>>> protocol_test_data
    {
        ...
        std::shared_ptr<hub_message_base>(new completion_message("1", "error", value(), false)) },

        // completion message with result
        { string_from_bytes({0x07, 0x95, 0x03, 0x80, 0xA1, 0x31, 0x03, 0x2A}),
        std::shared_ptr<hub_message_base>(new completion_message("1", "", value(42.f), true)) },

        // completion message with no result or error
        { string_from_bytes({0x06, 0x94, 0x03, 0x80, 0xA1, 0x31, 0x02}),
        std::shared_ptr<hub_message_base>(new completion_message("1", "", value(), false)) },
    };
}
...
TEST(messagepack_hub_protocol, write_message)
{
    for (auto& data : protocol_test_data)
    {
        ...
        for (auto c : output)
        {
            std::cout << "0x" << std::hex << (unsigned int)c;
            std::cout << ", ";
        }
        std::cout << std::endl;
