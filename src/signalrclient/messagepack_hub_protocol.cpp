...
namespace signalr
{
    ...
    void pack_messagepack(const signalr::value& v, msgpack::packer<msgpack::sbuffer>& packer)
    {
        switch (v.type())
        {
    ...
    std::string signalr::messagepack_hub_protocol::write_message(const hub_message_base* hub_message) const
    {
...
#pragma warning (disable: 4061)
        ...
        case message_type::invocation:
        {
            ...
            packer.pack_int(message_type::invocation);
            // Headers
            packer.pack_map(0);
...
#pragma warning (pop)
        ...
        std::string payload(sbuf.data(), sbuf.size());
        binary_message_formatter::write_length_prefix(payload);
        return payload;
    }

    // Can we avoid allocating? :(
    std::vector<std::unique_ptr<hub_message_base>> messagepack_hub_protocol::parse_messages(const std::string& message) const
    {
